"""Training pipeline for the CICIoT2023 MLP baseline model."""
...
def run_mlp_training(
    ...
    checkpoint_dir: Path = Path("checkpoints/mlp/real_ciciot2023/baseline_25_epochs"),
    output_dir: Path = Path("results/mlp/real_ciciot2023/baseline_25_epochs"),
    epochs: int = 25,
    batch_size: int = 1024,
    ...
    """Compile and fit the MLP baseline model on prepared datasets."""
    ...
    LOGGER.info("Loading MLP training streaming dataset from %s", train_path)
    LOGGER.info("Loading MLP validation streaming dataset from %s", val_path)
    ...
    if not fresh and completed_epochs > 0 and latest_checkpoint_path.exists():
        LOGGER.info("Resuming MLP model from %s (completed epoch %d)...", latest_checkpoint_path, completed_epochs)
        model = keras.models.load_model(str(latest_checkpoint_path))
    else:
        LOGGER.info("Building FRESH MLP baseline model initialization...")
    ...
    try:
        ...
        LOGGER.warning("TensorBoard callback skipped: %s", exc)
    ...
    try:
        ...
        print(start_msg.encode("ascii", "replace").decode("ascii"), flush=True)
    with open(task_output_log_path, "a", encoding="utf-8") as log_f:
        log_f.write(start_msg)
        log_f.flush()

    if completed_epochs < epochs:
        LOGGER.info("Starting MLP training run (%d to %d epoch(s), batch size %d)...", completed_epochs + 1, epochs, batch_size)
        history = model.fit(
            train_ds,
        ...
        LOGGER.info("MLP training already complete (%d epochs).", completed_epochs)
    ...
    LOGGER.info("Saved final MLP checkpoint to %s and %s", final_checkpoint_path, out_final_path)
    ...
    metadata = {
        ...
        "architecture_summary": (
            "Dense(256, ReLU) -> Dropout(0.05) -> Dense(128, ReLU) -> Dropout(0.05) -> Dense(64, ReLU) -> Dropout(0.05) -> Dense(7, Softmax)"
        ),
        "optimizer": "Adam",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the MLP baseline cyber attack detector.")
    ...
    parser.add_argument(
        "--checkpoint-dir",
        type=Path,
        default=Path("checkpoints/mlp/real_ciciot2023/baseline_25_epochs"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/mlp/real_ciciot2023/baseline_25_epochs"),
    )
...
def main() -> None:
    ...
    print(f"Best Epoch: {results['best_epoch']} (Val Acc: {results['best_val_acc']:.6f})")
