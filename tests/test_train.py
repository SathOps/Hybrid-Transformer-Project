"""Tests for the CNN-Transformer training pipeline."""
...
def test_system_info_reporting():
    info = get_system_info()
    assert "gpu_available" in info
    assert "ram_rss_mb" in info
    assert info["ram_rss_mb"] > 0


def test_set_random_seeds():
    set_random_seeds(123)
    val1 = np.random.rand()
    val_tf1 = tf.random.normal((1,)).numpy()[0]

    set_random_seeds(123)
    val2 = np.random.rand()
    val_tf2 = tf.random.normal((1,)).numpy()[0]

    assert val1 == val2
    assert val_tf1 == val_tf2


def test_train_pipeline_smoke_run(sample_dataset: Path, tmp_path: Path):
    ...
    run_config = json.loads(Path(results["run_config_json"]).read_text(encoding="utf-8"))
    assert run_config["optimizer"] == "Adam"
    assert run_config["learning_rate"] == 0.00005
    assert run_config["batch_size"] == 8
    assert run_config["epochs_requested"] == 1
    assert run_config["early_stopping_enabled"] is False
